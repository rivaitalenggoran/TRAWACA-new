<?php
include '../backend/peneliti_minat_backend.php';
...
$query = "SELECT id_minat FROM minat_peneliti";
$stmt = $pdo->prepare($query);
$stmt->execute();
$ID_minat = $stmt->fetchAll(PDO::FETCH_ASSOC);
// UNTUK DATA MULTILINGUAL --------------------------
?>
...
<div class="sidebar" id="sidebar">
    ...
    <a href="minat_dashboard.php?id_peneliti=<?= $id_peneliti; ?>" class="active">
            ...
            <div class="header p-3 mb-4" style="background-color: #DCD1B8; border-radius: 5px;">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Minat</li>
                    </ol>
                </nav>
            </div>

            <!----------------------- Minat Content --------------------------------------------------------------------->
            <h3 class="mb-1">Tabel Minat</h3>

            <a href="CRU/minat_tambah.php" class="btn btn-primary mb-3">Tambah Minat</a>

            <select onchange="window.location.href=this.value;">
                <option value="">PILIH ID MINAT UNTUK TAMBAH BAHASA MINAT</option>
                <?php foreach ($ID_minat as $minat): ?>
                    <option value="CRU/minat_tambah_bahasa.php?id_minat=<?php echo $minat['id_minat']; ?>">
                    <?php echo $minat['id_minat']; ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <table class="table table-bordered">
                <thead class="table-dark">
                    <tr>
                        ...
                        <th>Kode Bahasa</th>
                        <th>Nama Minat</th>
                        <th>Sub Topik Minat</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
        <?php if ($selected_minat): ?>
                <tbody>
                <?php foreach ($selected_minat as $row) : ?>  
                    <tr>                
                        <td><?= $row['id_minat']; ?></td>
                        <td><?= $row['id_bahasa']; ?></td>
                        <td><?= $row['kode_bahasa']; ?></td>
                        <td><?= $row['nama_minat']; ?></td>
                        <td><?= $row['subtopik_minat']; ?></td>
                        <td>
                            <a href="CRU/minat_edit.php?edit=<?= urlencode($row['id_bahasa']); ?>" class="btn btn-warning btn-sm">Edit</a>
                            <a href="?deleteminat=<?= $row['id_bahasa']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus?')">Hapus</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
